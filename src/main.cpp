...
#if defined(NDEBUG)
# error "Bitnewcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x14bea69d085fec427c88159cf415efb5114bee269cd5dd4928a6d5faec323933");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitnewcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitnewcoin Signed Message:\n";
...
{
    // Bitnewcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bitnewcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bitnewcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bitnewcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitnewcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitnewcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe8c81e7c274db6a0f89642ef5fd68539745004ebf0fd230b17a169536d0f1455");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0436ff1adeb60ca6d0b31a3b6c11a18c365586219f8ce6537026d703861fe8904a46aa43eb17c9ea4dadf3b9485d96ac9acc281da77b7565700b2dd75278caea43") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515204911;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085464633;

        if (fTestNet)
        {
            block.nTime    = 1515204738;
            block.nNonce   = 385771455;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x50a2cb6c2bdfbfebe09c85a516fd7bb493e15af3a5180fbf1895013d96d18b03"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc9, 0xbc, 0xd5 }; // Bitnewcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitnewcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitnewcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitnewcoinMiner(CWallet *pwallet)
{
    printf("BitnewcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitnewcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitnewcoinMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitnewcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitnewcoinMiner, pwallet));
}
